...
void WriteStudentss(const string& filename, const Student* students, const int* indexArray, int N);
...
void PrintIndexSorted(const Student* students, const int* indexArray, int N);
bool BinarySearch(const Student* students, int N, const string& surname, int course, int specialized_grade);
int CalculateSpecializedGrade(const Student& student);
void SortForBinarySearch(Student* students, int N);
...
int main() {
    ...
    string filename;
    cout << "Enter filename for student data storage: ";
    cin >> filename;
    ...
    do {
        ...
        cout << "[3] - Sort information physically and write to file" << endl;
        cout << "[4] - Read from file and display index-sorted information" << endl;
        ...
        case 3:
            ...
            cout << "Information sorted and written to file." << endl;
            break;
        case 4: {
            int* indexArray = IndexSort(students, N);  // Сортування студентів
            PrintIndexSorted(students, indexArray, N);  // Виведення відсортованої таблиці
            WriteStudentss("sorted_students.bin", students, indexArray, N);  // Запис в файл
            delete[] indexArray;
            break;
        } break;
        case 5:
        {
            ...
            int searchCourse, searchSpecializedGrade;
            cout << "Enter student's surname to search: ";
            cin >> searchSurname;
            cout << "Enter student's course to search: ";
            cin >> searchCourse;
            cout << "Enter student's specialized grade to search: ";
            cin >> searchSpecializedGrade;

            SortForBinarySearch(students, N);
            bool found = BinarySearch(students, N, searchSurname, searchCourse, searchSpecializedGrade);
            if (found)
...
void CreateStudents(Student* students, const int N) {
    for (int i = 0; i < N; ++i) {
        ...
        int specialization;
        while (!(cin >> specialization) || specialization < 0 || specialization > 4) {
        ...
        if (students[i].specialization == CS) {
            ...
            while (!(cin >> students[i].additional_grade.programming)) {
                cout << "Invalid input. Please enter a valid number for programming grade: ";
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
            }
        }
            ...
            while (!(cin >> students[i].additional_grade.numerical_methods)) {
                cout << "Invalid input. Please enter a valid number for numerical methods grade: ";
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
            }
        }
            ...
            cout << "Pedagogy grade: ";
            while (!(cin >> students[i].additional_grade.pedagogy)) {
                cout << "Invalid input. Please enter a valid number for pedagogy grade: ";
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
            }
        }
...
void WriteStudents(const string& filename, const Student* students, int N) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        case CS: specName = "Computer Science"; break;
        case IT: specName = "Informatics"; break;
        case ME: specName = "Math and Economics"; break;
        case PHI: specName = "Physics and Informatics"; break;
        case TECH: specName = "Labor Training"; break;
        }
        file << setw(28) << left << specName << " | ";
        file << setw(7) << right << students[i].physics_grade << " | ";
        file << setw(11) << right << students[i].math_grade << " | ";
        switch (students[i].specialization) {
        case CS: file << setw(16) << right << students[i].additional_grade.programming << " |" << endl; break;
        case IT: file << setw(16) << right << students[i].additional_grade.numerical_methods << " |" << endl; break;
...
void WriteStudentss(const string& filename, const Student* students, const int* indexArray, int N) {
    ...
    for (int i = 0; i < N; ++i) {
        const Student& student = students[indexArray[i]];
        file << "| " << setw(2) << right << i + 1 << " | ";
        file << setw(13) << left << student.surname << " | ";
        file << setw(6) << right << student.course << " | ";
        string specName = specializationNames[student.specialization];
        file << setw(28) << left << specName << " | ";
        file << setw(7) << right << student.physics_grade << " | ";
        file << setw(11) << right << student.math_grade << " | ";

        switch (student.specialization) {
        case CS:
            file << setw(16) << right << student.additional_grade.programming << " |" << endl;
            break;
        case IT:
            file << setw(16) << right << student.additional_grade.numerical_methods << " |" << endl;
            break;
        default:
            file << setw(16) << right << student.additional_grade.pedagogy << " |" << endl;
            break;
        }
    }
    file << "=======================================================================================================" << endl;
